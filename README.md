⭐ Anupam Dhoundiyal
👋 Hi, I'm Anupam

C++ & Python Developer • Linux Enthusiast • Learning Ethical Hacking & Kernel Development

🚀 About Me

🔧 I like building tools, scripts, and solving system-level problems

🧵 Currently learning Kernel Development & Cybersecurity Basics

🐧 Daily driver: Linux (Fedora)

🎯 Goal: Improve low-level skills, contribute to open-source, and build powerful system utilities

⚡ Fun fact: I break things to understand how they work

🛠️ Skills

Languages:

C++, Python, Bash

Tech & Tools:

Linux, Git, VS Code, GCC/Clang

Kernel Internals (beginner)

Basic Ethical Hacking (Nmap, Wireshark, Burp Suite, etc.)


📫 Contact

✉️ Email: anupam.dhoundiyal123@gmail.com


⭐ Support

If you like my work, consider giving a ⭐ to my repositories!
